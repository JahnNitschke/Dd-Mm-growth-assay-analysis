---
title: "Normalized residual growth for experiment XYZ"
author: "JN"
date: "`r date()`"
output:
...
#rfu = random fluorescence unit, amoebae growth

#If, for example, you use another measurement protocol at the plate reader, adapt the first section, where it finds the two measurement blocks of luminescence and fluorescence by looking for specific strings
...
#initialize a container variable for qc plots
qcplot_collect <- data.frame(matrix(ncol = 10, nrow = 0))
colnames(qcplot_collect) <- c("rlu_auc", "rfu_auc", "rlu_sauc", "rfu_sauc", "plate_row", "plate_col", "cond", "ctrl_type", "plate", "br")
...
for (k in seq(1, length(all_files))) {
  ...
  s_row <- which(raw_sheet$`Software Version` == "Read 1:Lum") - 2
  ...
  #find fluorescence data block
  #cols are the same as for lum data
  s_col_cherry <- s_col
  e_col_cherry <- e_col
  #start row cherry
  s_row_cherry <- which(raw_sheet$`Software Version` == "Read 2:580,610") - 2
  #end row cherry
  found <- F
  c <- 0
  while (found == F) {
    if  (anyNA(raw_sheet[s_row_cherry+c, s_col_cherry])){
      found <- T
    } else {
        e_row_cherry <- s_row_cherry+c
    }
    c <- c + 1
  }
  
  
  #check equal lenghts of reading windows, should be all TRUE
  #e_row-s_row == e_row_cherry-s_row_cherry
  
  
  #subset the lum data block from the raw_sheet
  ...
  #subset the cherry data block from the raw_sheet
  raw_data_tmp <- data.frame(matrix(nrow = unique(e_row-s_row)+1, ncol = 0))
  for (i in seq(1, length(s_row))) {
    raw_data_tmp <- cbind(raw_data_tmp, raw_sheet[s_row_cherry[i]:e_row_cherry[i], s_col:e_col])
  }
  raw_data_rfu <- raw_data_tmp
  
  #get timepoints from the raw_sheet
  ...
  raw_data_rfu_t <- t(raw_data_rfu) %>% as_tibble()
  raw_data_rfu_t_numeric <- raw_data_rfu_t %>% 
    mutate_at(6:ncol(raw_data_rfu_t), as.numeric) %>%
    rename(vc = V1,
           br = V2,
           conc = V3,
           cond = V4,
           well = V5) %>%
    rename_with(~ as.character(seq(1:(ncol(raw_data_rfu_t)-5))), all_of(colnames(raw_data_rfu_t)[6:ncol(raw_data_rfu_t)])) %>% #rename timepoints columns
    mutate_at(c("br", "conc"), as.numeric) %>% #make br and conc numeric
    select(1:ncol(raw_data_rfu_t)) %>% #use this line to crop column/timepoints if you want
    group_by(vc, br, conc, cond, well) %>%
    arrange(cond, conc, br)
    
  rlu_data_num_sorted <- t(raw_data_rlu_t_numeric[,6:ncol(raw_data_rlu_t_numeric)])
  rfu_data_num_sorted <- t(raw_data_rfu_t_numeric[,6:ncol(raw_data_rfu_t_numeric)])
  ...
  heatmap.2(x = rlu_data_num_sorted,
            ...
            labCol = paste0(raw_data_rlu_t_numeric$cond, ", ", round(raw_data_rlu_t_numeric$conc, 4), ", ", raw_data_rlu_t_numeric$br, ", ", raw_data_rlu_t_numeric$well),
            offsetCol = -0.4,
            offsetRow = -0.4,
  ...
  #heatmap for rfu data
  pdf(paste0(output, "02_RFU_tmp_", all_files_filenames[k], ".pdf"), width = 100, height = 50)
  heatmap.2(rfu_data_num_sorted,
            cellnote = rfu_data_num_sorted,
            notecex = 0.5,
            notecol = "black",
            ...
            main = paste("RFU 490:510 heatmap:", paste0("plate ", k)),
            cex.main = 10
            )
  dev.off()
  ...
  for (i in seq(1, length(timepoints_n)-1)) { #loop over timepoints
  ...
  #calculate AUC (trapezoids between timepoints) for every column for rfu
  for (i in seq(1, length(timepoints_n)-1)) {
    j <- i+1 #index of next timepoint
    if (i == 1) { #compute auc of timepoints 1 & 2, then accumulate following timepoints
      auc_tmp <- ((rfu_data_num_sorted[i,]+rfu_data_num_sorted[j,])/2)*(timepoints_n[j]-timepoints_n[i])
    } else {
      auc_tmp <- auc_tmp + ((rfu_data_num_sorted[i,]+rfu_data_num_sorted[j,])/2)*(timepoints_n[j]-timepoints_n[i])
    }
  }
  auc_rfu <- auc_tmp
  ...
  ctrls_sry_br_empty <- data.frame(matrix(nrow = 2*(length(all_vc) + 1), ncol = 6))
  colnames(ctrls_sry_br_empty) <- c("ctrl", "readout", "mean", "sd", "median", "mad")
  #initialise labels in first two cols
  ctrls_sry_br_empty$ctrl <- rep(c(all_vc, "PC Rif"), 2)
  ctrls_sry_br_empty$readout <- c(rep("RLU", length(all_vc) + 1), rep("RFU", length(all_vc) + 1)) 
  ...
  sauc_rfu <- matrix(nrow = 0, ncol = 1) 
  sauc_rfu_sd <- matrix(nrow = 0, ncol = 1)
  ...
  for( h in seq(min(all_br), max(all_br))){ #loop over brs present in this plate
    ...
    auc_rfu_br <- auc_rfu[meta_data_sorted$br == h]
    
    #create ctrls lookup table for this br
    ctrls_sry_br <- ctrls_sry_br_empty
    for(i in seq(1, nrow(ctrls_sry_br)/2)){ #loop over VCs in ctrl_sry table
      ...
      #which is the corresponding row for RFU 
      j <- i + (nrow(ctrls_sry_br)/2)
      ctrls_sry_br$mean[j] <- mean(auc_rfu_br[these_vc_idx])
      ctrls_sry_br$median[j] <- median(auc_rfu_br[these_vc_idx])
      ctrls_sry_br$sd[j] <- sd(auc_rfu_br[these_vc_idx])
      ctrls_sry_br$mad[j] <- mad(auc_rfu_br[these_vc_idx])
    }
    
    #append a t0 baseline ctrl
    ctrls_sry_br <- rbind(ctrls_sry_br, c("PC baseline", "RFU", 
                                  sum(median(rfu_data_num_sorted[1,])*diff(timepoints_n)), 0, 
                                  sum(median(rfu_data_num_sorted[1,])*diff(timepoints_n)), 0))
    ...
    #scale auc of rfu by the medians of nc and pc baseline
    sauc_rfu_br <- matrix(nrow = length(auc), ncol = 0)
    sauc_rfu_br_sd <- matrix(nrow = length(auc), ncol = 0)
  
    for (i in seq(1, length(all_vc))){
      this_vc <- all_vc[i]
      ...
      this_vc_row <- which(ctrls_sry_br$ctrl == this_vc & ctrls_sry_br$readout == "RFU")
      this_pc_row <- which(ctrls_sry_br$ctrl == "PC baseline" & ctrls_sry_br$readout == "RFU")
    
      sauc_rfu_br[these_idx] <- (auc_rfu_br[these_idx]-ctrls_sry_br$median[this_pc_row]) * (1/(ctrls_sry_br$median[this_vc_row]-ctrls_sry_br$median[this_pc_row]))
    
      #error propagation
      #get uncertainty by taking the mad of the nc and scale it to the median
      sauc_rfu_br_sd[these_idx] <- ctrls_sry_br$mad[this_vc_row]/ctrls_sry_br$median[this_vc_row]
    }
    
    #collect br saucs
    sauc_rfu <- c(sauc_rfu, sauc_rfu_br)
    sauc_rfu_sd <- c(sauc_rfu_sd, sauc_rfu_br_sd)
  ...
  #QC plots
  qcplot <- data.frame(matrix(ncol = 10, nrow = length(auc)))
  colnames(qcplot) <- c("rlu_auc", "rfu_auc", "rlu_sauc", "rfu_sauc", "plate_row", "plate_col", "cond", "ctrl_type", "plate", "br")
  
  wp_row <- substr(as.character(meta_data_sorted$well),1,1)
  wp_col <- substr(as.character(meta_data_sorted$well),2,3)
  
  qcplot$rlu_auc <- auc
  qcplot$rfu_auc <- auc_rfu
  qcplot$rlu_sauc <- sauc_rlu
  qcplot$rfu_sauc <- sauc_rfu
  ...
  p <- ggplot(data = qcplot, aes(rfu_auc, rlu_auc, color= ctrl_type)) +
  geom_point(alpha=0.5, size=2) 
  print(p)
  
  #normalized, ctrls are colored
  p <- ggplot(data = qcplot, aes(rfu_sauc, rlu_sauc, color= ctrl_type)) +
  geom_point(alpha=0.5, size=2) 
  print(p)
  ...
  #rfu unnormalized vs normalized, ctrls are colored
  p <- ggplot(data = qcplot, aes(rfu_auc, rfu_sauc, color= ctrl_type)) +
  geom_point(alpha=0.5, size=2)
  print(p)
  
  #rfu vs rlu normalized, plate cols are colores
  p <- ggplot(data = qcplot, aes(rfu_sauc, rlu_sauc, color= plate_col)) +
  geom_point(alpha=0.5, size=2) 
  print(p)
  
  #rfu vs rlu normalized, plate rows are colores
  p <- ggplot(data = qcplot, aes(rfu_sauc, rlu_sauc, color= plate_row)) +
  geom_point(alpha=0.5, size=2) 
  print(p)
  ...
  names(auc_rfu) <- as.character(seq(1,nrow(timepoints)))
  
  #collect parameters for output 
  out <- list(sauc_rlu, sauc_rlu_sd,
              sauc_rfu, sauc_rfu_sd,
              rlu_data_num_sorted, rfu_data_num_sorted, 
  ...
  names(out) <- c("scaled_auc_mean", "scaled_auc_sd", 
                  "scaled_auc_rfu", "scaled_auc_rfu_sd",
                  "rlu_data_sorted", "rfu_data_sorted", 
                  "meta_data_sorted", "auc", 
                  "auc_rfu", 
                  "timepoints", "zfactor_rlu",
                  "filename",
                  "qcplot")
  ...
  dose_conds_rfu <- sauc_rfu[!is.na(meta_data_sorted$conc)]
  dose_conds_sry <- cbind(dose_conds_meta, dose_conds_rlu, dose_conds_rfu)
  ...
  #calculate medians per group, over bioreps
  dose_conds_br_mean <- dose_conds_sry %>%
    group_by(br_label) %>%
    summarise_at(vars(dose_conds_rlu, dose_conds_rfu), list(name = median, mad, mean, sd))
  ...
  dose_conds_br_mean <- dose_conds_br_mean %>% 
    rename(Mm_in_infection_median = dose_conds_rlu_name,
           Dd_in_infection_median = dose_conds_rfu_name,
           Mm_in_infection_mad = dose_conds_rlu_fn1,
           Dd_in_infection_mad = dose_conds_rfu_fn1,
           Mm_in_infection_mean = dose_conds_rlu_fn2,
           Dd_in_infection_mean = dose_conds_rfu_fn2,
           Mm_in_infection_sd = dose_conds_rlu_fn3,
           Dd_in_infection_sd = dose_conds_rfu_fn3
           )
  ...
  dose_conds_br_mean <- relocate(dose_conds_br_mean, log10conc, .before = Mm_in_infection_median)
  
  dose_conds_tr_mean$log10conc <- log(dose_conds_tr_mean$conc, 10)
  dose_conds_tr_mean <- relocate(dose_conds_tr_mean, log10conc, .before = Mm_in_infection_median)
  ...
  for (i in seq(1, length(all_conds))) { #loop over conditions and output a plot
    ...
    p <- ggplot(data = df2plot, aes(log10conc, Mm_in_infection_median, shape = cond)) +
      geom_point(alpha=0.5, size=2) +
      geom_errorbar(aes(ymin=Mm_in_infection_median-Mm_in_infection_mad, ymax=Mm_in_infection_median+Mm_in_infection_mad),
                    width = 0.1) +
      ylim(-1, 2.0) +
      ggtitle(paste(this_cond, ": RLU dose response curve"))
    print(p)
  
    p <- ggplot(data = df2plot, aes(log10conc, Dd_in_infection_median, shape = cond)) +
      geom_point(alpha=0.5, size=2) +
      geom_errorbar(aes(ymin=Dd_in_infection_median-Dd_in_infection_mad, ymax=Dd_in_infection_median+Dd_in_infection_mad),
                    width = 0.1) +
      ylim(-1, 2.0) +
      ggtitle(paste(this_cond, ": RFU dose response curve"))
    print(p)
  ...
  out_all_trs <- tibble(dose_conds_sry[,c(1:7, 10)]) %>%
...
```{r, echo = FALSE}
 ...
 #unnormalized, ctrls are colored
  p <- ggplot(data = qcplot_collect, aes(rfu_auc, rlu_auc, color= ctrl_type, shape = plate)) +
  geom_point(alpha=0.5, size=2) 
  print(p)
  
  #normalized, ctrls are colored
  p <- ggplot(data = qcplot_collect, aes(rfu_sauc, rlu_sauc, color= ctrl_type, shape = plate)) +
  geom_point(alpha=0.5, size=2) 
  print(p)
  ...
  #rfu unnormalized vs normalized, ctrls are colored
  p <- ggplot(data = qcplot_collect, aes(rfu_auc, rfu_sauc, color= ctrl_type, shape = plate)) +
  geom_point(alpha=0.5, size=2)
  print(p)
  
  #rfu vs rlu normalized, plate cols are colores
  p <- ggplot(data = qcplot_collect, aes(rfu_sauc, rlu_sauc, color= plate_col, shape = plate)) +
  geom_point(alpha=0.5, size=2) 
  print(p)
  
  #rfu vs rlu normalized, plate rows are colores
  p <- ggplot(data = qcplot_collect, aes(rfu_sauc, rlu_sauc, color= plate_row, shape = plate)) +
  geom_point(alpha=0.5, size=2) 
  print(p)
  
  #rfu vs rlu normalized, plate rows are colores
  p <- ggplot(data = qcplot_collect, aes(rfu_sauc, rlu_sauc, color= plate, shape = ctrl_type)) +
  geom_point(alpha=0.5, size=2) 
  print(p)
  
  #rfu vs rlu normalized, plate rows are colores
  p <- ggplot(data = qcplot_collect, aes(rlu_auc, rlu_sauc, color= plate, shape = ctrl_type)) +
  geom_point(alpha=0.5, size=2) 
  print(p)
  
  #rfu vs rlu normalized, plate rows are colores
  p <- ggplot(data = qcplot_collect, aes(rfu_auc, rfu_sauc, color= plate, shape = ctrl_type)) +
  geom_point(alpha=0.5, size=2) 
  print(p)
